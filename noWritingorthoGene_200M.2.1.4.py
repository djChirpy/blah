...
#   Parameters   #
################################################
####RossEdit to link gene name for gtf
try:
    geneName = str(sys.argv[10])
except:
    geneName = 'geneName'
...
'''
xspGuiedFn is list of species names, genome builds, etc. as well as wget path to download, some naming conventions don't match
between downloaded genomes and those in cactus. Does that indicate potential coordinate problems? My version uses the genomes
exported directly from the HAL, so the versioning has to be the same, which feels maybe safer
'''
...
'''just gives location of gtf and ortholog lookup files for mouse and capra hircus (sp)'''
...
'''
halInfoM ends up as a list of lists, one per line of the genome description csv file.
With each line's entries split into a list for that line.
'''
################################################
...
halInfoM = [];
'''halInfoM now being reset? So above seems pointless'''
...
namingConventionMasterD = {"GenBank_to_RefSeq":"GenbankToRefseqName",
                           "GenBank_to_UCSC":"GenbankToUCSCName",
                           "GenBank_to_UCSC":"GenbankToUCSCName",
                           "Sequence-Name_to_GenBank":"SequenceNameToGenbankName"};

curRow = 0;

'''
reading through the same csv as above, again
'''
for curLine in fileinput.input([xspecGuideFn]): #Read in list of things for gene distance
    curLineP = curLine.rstrip().split(",");
    if(curRow > 0 and curLineP[0] != '') :
        ...
        curSpec = curLineP[1]; #species name string as it appears in hal

        halInfoM.append(curLineP) #addding line elements to a list of lists again, as above
        genomeSourceD[curSpec] = curLineP[9]; #chromosome naming convention in fasta file

        if(curSpec == referenceSpecies) :
            referenceGenomeFn = curLineP[11]; #location of genome .fna file (think this is the same as fasta? - FastaNucleicAcid >.<)

        genomeFastaD[curSpec] = curLineP[11];
        spec2indD[curSpec] = curRow - 1; #just indexing what line the species info is on, I believe
...
'''
builds a dictionary of between-species distances: probably keep or adapt
result is specDistD, which is a dictionary with the keys being species_name-species2_name
and then entry being the pairwise distance
'''
...
'''boolean set to true (create bed file for ref species, usually human)/false (don't)
'''
if gtf2BedB :
...
'''
Everythinng above is to get to reference protein sequence, I think.
'''
...
if anchorOverB :
    ...
    for curLineP in anchorSpecM :
        ...
        callHLOStr = "halLiftover %s Homo_sapiens %s %s %s" % (halFn,curProtBedFn,curSpecName,hloOutbedFn);
...
if halOverB :
    ...
    for curSpecLineP in halInfoM :
        ...
        if( not curSpecName in useAnchorP) :
            ...
            callHLOStr = "halLiftover %s %s %s %s %s" % (halFn,curAnchorSpec,curAchorBedFn,curSpecName,hloOutbedFn);
            #print(callHLOStr);
...
if getSeqB:
    ...
    #for curSpecLineP in halInfoM[1:10] :
        ...
        if(curSpecName in halSpeciesNamesP) :
            ...
            #print('curSpecSummaryP: ' + str(curSpecSummaryP))
            ...
            if(not curSpecLineP[8].startswith("N/A") and not curSpecLineP[8] == "" and path.exists(curSpecFasta)) :
                ...
                #print('curHalBedM = ' + str(curHalBedM))
                ...
                for curIoLine in io.TextIOWrapper(process.stdout, encoding="utf-8"):
                    #print(line.rstrip());
                    if(not curIoLine.startswith(">")) :
                        '''grab sequence for an entire interval in the bed file'''
                        newSpecCdsSeq = Seq(curIoLine.rstrip(),ambiguous_dna); # grab FASTA line by line
                        curStrand = curHalBedM[curBedPos][5]; #+/- strand from bed file
                        currentExon = curHalBedM[curBedPos]
                        workingExon.append(currentExon)
                        ...
                        curEndExonB = False;
                        '''assume that you're not at the end of an exon'''
                        #print(curBedPos);

                        if( (curBedPos+2) > len(curHalBedM)) :
                            '''if this is the last entry in the bed file, assume that this is the end of an exon'''
                            curEndExonB = True;
                            #workingExon.append(curHalBedM[curBedPos]) #This test will be true if the current sequence is from the last exon
                            # in the bed, but how do we know we have the entire sequence from that exon? Answer: 
                            # by default bedtools doesn't separate sequence into standard fasta-length lines,
                            # so I think every time a line is grabbed, that's an entire "exon", or more
                            # specifically in this case, and entire interval, which should be an exon, but could
                            # be otherwise?
                        else :
                            if (curHalBedM[curBedPos][3] != curHalBedM[curBedPos+1][3]) :
                                #workingExon.append(curHalBedM[curBedPos]) #New exon or at the end
                                curEndExonB = True;
                                '''if the next entry in the bed file is labelled as a different exon, end the exon with this entry'''
                            #else:
                                ''''I think the interval should be appended and treated as part of a single exon
                                    one exon, comprised of many intervals'''
                                
                                #workingExon.append(curHalBedM[curBedPos])
                                #print('ISTHISEVERFALSE')
                                #print('should I append this? ' + str(curHalBedM[curBedPos]))
                                # This is testing to see if the next interval in the bed file has the same name
                                # which would mean it's from the same exon
                                # this usually should be one line/one exon
                        #read sequence from fasta and add to exon until exon length matches expected
                        #this represents the sequence of that exon
                        if(curEndExonB) :
                            #workingExon.append(curHalBedM[curBedPos])
                            #exonHal = curHalBedM[curBedPos]
                            ...
                            #compare exon sequence length to anchor species exon length
                            ...
                            print('anchor length: ' + str(curAnchorNucNum))
                            curMergeNucNum = int(len(testSpecCdsSeq)); #Nucleotide length of the merge sequence
                            print('merged length: ' + str(curMergeNucNum))
                            curMergeNucDiff = curAnchorNucNum - curMergeNucNum;
                            curNumMergeBad = 0;
                            ####RossEdits
                            #currentInterval = [int(workingExon[0][1]), int(workingExon[(len(workingExon) - 1)][2])]
                            currentInterval = [int(currentExon[1]), int(currentExon[2])]
                            #currentInterval = [int(curHalBedM[curBedPos][1]), int(curHalBedM[curBedPos][2])]
                            #print('current interval : ' + str(currentInterval))
                            #if exon is not the same length as the anchor exon, try to adjust
                            #question the logic here, I think codons can span exons?
                            #actually, nevermind. If the difference is 3, that indicates that an
                            #amino acid has been removed or added, if it is not 3, that suggests
                            #that there's an assembly error, missing or adding bases. 
                            #technically, an amino acid could be added at the splice
                            #site, so it could span the splice, if the rest was
                            #added on the other side of the splice, but unlikely
                            adjustedMergedExon = []
                            adjustedNonMergedExon = []
                            if( not (curMergeNucDiff % 3) == 0 ) : #Try to make adjustments if exon frame changes
                                print('shifting_exons')
                                curReviseTestSeq1 = "";
                                curReviseTestSeq2 = "";
                                ...
                                if ( (curMergeNucDiff % 3) == 1) : # Remove two nucleotides to make things work (from begin or end)
                                    ...
                                    ####RossEdits
                                    #first revision is removing 2 bases from beginning of exon,
                                    #which would equal first 2 on positive interval
                                    #or last two on negative interval
                                    if curStrand == '+':
                                        testInterval1 = [(currentInterval[0] + 2), currentInterval[1]]
                                        testInterval2 = [currentInterval[0], (currentInterval[1] - 2)]
                                    else:
                                        testInterval1 = [currentInterval[0], (currentInterval[1] - 2)]
                                        testInterval2 = [(currentInterval[0] + 2), currentInterval[1]]
                                ...
                                if ( (curMergeNucDiff % 3) == 2) : # Remove two nucleotides to make things work (from begin or end)
                                    ...
                                    ####RossEdit
                                    if curStrand == '+':
                                        testInterval1 = [(currentInterval[0] + 1), currentInterval[1]]
                                        testInterval2 = [currentInterval[0], (currentInterval[1] - 1)]
                                    else:
                                        testInterval1 = [currentInterval[0], (currentInterval[1] - 1)]
                                        testInterval2 = [(currentInterval[0] + 1), currentInterval[1]] 
                                    ###    
                                ...
                                if(curNumBad1 < curNumBad2) : #Remove from beginning if you reduce stop codons
                                    ...
                                    ####RossEdit
                                    testInterval = testInterval1
                                    ###
                                    curNumMergeBad = curNumBad1;
                                else :
                                    testSpecCdsSeq = curReviseTestSeq2;
                                    ####RossEdit
                                    testInterval = testInterval2
                                    ###
                                    curNumMergeBad = curNumBad2;
                                ###RossEdit --- seems to work! need to output this updated interval
                                #as part of a full bed entry. Probably store the intervals (exons) as they
                                #come in, or write on the fly. Also still need to work through the adjustment
                                #in the next step and test to see if intervals actually
                                #correspond to the final sequence the original script makes
                                #print('updated interval: ' + str(testInterval))
                                currentExon[1] = testInterval[0]
                                currentExon[2] = testInterval[1]
                                #workingExon[0][1] = testInterval[0]
                                #workingExon[len(workingExon) -1][2] = testInterval[1]
                                #print('workingExon: ' + str(workingExon))
                                #adjustedMergedExon = [workingExon[0][0],
                                #                      testInterval[0],
                                #                      workingExon[len(workingExon) -1][]]
                                #adjustedMergedExon = [curHalBedM[curBedPos][0],
                                #                      testInterval[0],
                                #                      testInterval[1],
                                #                      curHalBedM[curBedPos][3],
                                #                      curHalBedM[curBedPos][4],
                                #                      curHalBedM[curBedPos][5]]
                            
                            #else:
                            ##########################3
                            ######Just moved this outside of the ifelse canceled above
                            adjustedMergedExon = [currentExon]
                                #adjustedMergedExon = curHalBedM[curBedPos]
                            curExon = curHalBedM[curBedPos][3];
                            curExonIden = curSpecName + "_" + curExon;
                            print('adjustedMergedExon' + str(adjustedMergedExon))

                            if curExonIden in mergedExonsP : #If exon had been merged, pull up original sequence
                                print('merged exon: ' + curExonIden)

                                curExonSepBedM = halSpecBedSepD[curExonIden];
                                #print('curExonSepBedM: ' + str(curExonSepBedM))
                                curSepSeq = "";
                                for curSepExonRowP in curExonSepBedM :
                                    #print('curSepExonRowP = ' + str(curSepExonRowP))
                                    curAdjStart = int(curSepExonRowP[1]) - int(curHalBedM[curBedPos][1]);
                                    #print('curAdjStart: ' + str(curAdjStart))
                                    curAdjStop = int(curSepExonRowP[2]) - int(curHalBedM[curBedPos][1]);
                                    #print('curAdjStop: ' + str(curAdjStart))
                                    #print('curExonSeq: ' + str(curExonSeq))
                                    curSepSeq  = curSepSeq + curExonSeq[curAdjStart:curAdjStop]; #Subset non rev-comp
                                    #print('curSepSeq: ' + str(curSepSeq))
                                ...
                                for entry in curExonSepBedM:
                                    testIntervals1.append([entry[0], int(entry[1]), int(entry[2]), entry[3], entry[4], entry[5]])
                                    testIntervals2.append([entry[0], int(entry[1]), int(entry[2]), entry[3], entry[4], entry[5]]) 
                                ...
                                if( not (curSepNucSepDiff % 3) == 0 ) : #Try to make adjustments if exon frame changes
                                    #print('modifying the pre-merged sequences, starting with: ' + str(testIntervals1))
                                    curReviseTestSeq1 = "";
                                    curReviseTestSeq2 = "";
                                    if ( (curSepNucSepDiff % 3) == 1) : # Remove two nucleotides to make things work (from begin or end)
                                        curReviseTestSeq1 = curSepSeqTest[2:];
                                        #RossEdit - intending to remove 2 bases from beginning of seq
                                        #would be the equivalent of adding two to the first interval of a positive seq
                                        #or removing two from the last interval of a negative seq
                                        if curStrand == '+':
                                            testIntervals1[0][1] += 2
                                        else:
                                            testIntervals1[len(testIntervals1) - 1][2] -= 2
                                        curReviseTestSeq2 = curSepSeqTest[:-2];
                                        #RossEdit - intending to remove 2 bases from the end of seq
                                        #would be the equivalent of removing 2 from the end of the last interval
                                        #in a positive seq, or adding 2 to the start of the first interval in a negative seq
                                        if curStrand == '+':
                                            testIntervals2[len(testIntervals2) - 1][2] -= 2
                                        else:
                                            testIntervals2[0][1] += 2
                                    ...
                                    if ( (curSepNucSepDiff % 3) == 2) : # Remove two nucleotides to make things work (from begin or end)
                                        ...
                                        ###
                                        if curStrand == '+':
                                            testIntervals1[0][1] += 1
                                            testIntervals2[len(testIntervals2) - 1][2] -= 1
                                        else:
                                            testIntervals1[len(testIntervals1) - 1][2] -= 1
                                            testIntervals2[0][1] += 1
                                        ###
                                    #print('test interval 1: ' + str(testIntervals1))
                                    #print('test interval 2: ' + str(testIntervals2))    
                                    curTmpCds1 = curSpecCdsSeq + curReviseTestSeq1;
                                    curTmpCds2 = curSpecCdsSeq + curReviseTestSeq2;
                                    curTmpProt1 = curTmpCds1.translate();
                                    curTmpProt2 = curTmpCds2.translate();
                                    ...
                                    if(curNumSepBad1 < curNumSepBad2) : #Remove from beginning if you reduce stop codons
                                        ...
                                        useInterval = testIntervals1
                                    else :
                                        curSepSeqTest = curReviseTestSeq2;
                                        curNumSepBad = curNumSepBad2;
                                        useInterval = testIntervals2
                                else:
                                    useInterval = testIntervals1   
                                print('useInterval: ' + str(useInterval))
                                ...
                                if(curSepNumBad <  curMergeNumBad) : #Assume that you are moving onto the next exon and add the sequence
                                    useSepB = True;
                                    for interval in useInterval:
                                        print('adding interval: ' + str(interval))
                                        exonBed.append(interval)
                                    print(curSpec + ': UsingTheOriginalNonMergedExon')
                                    ...
                                    for entry in adjustedMergedExon:    
                                        exonBed.append(entry)
                                    workingExon = []   
                                ...
                                #RossEdit
                                for entry in adjustedMergedExon:
                                    #print('non merged entry: ' + str(entry))
                                    exonBed.append(entry)
                                workingExon = []
                                #
                            ...
                            curExonSeq = curExonSeq + newSpecCdsSeq; #If not new exon or end, just store seq
                            #RossEdit
                            #exonBed.append(curHalBedM[curBedPos])
                            #workingExon.append(curHalBedM[curBedPos])
                            #exonBed.append(exonHal)
                            #
                ...
                #print('Final_bed entries: ' + str(exonBed))
                ...
                if(printCurProtB):
                    if not os.path.exists('/seq/vgb/swofford/zoonomia/wholeGenomeAnnotation/orthGeneOut/gtfOut/' + curSpecName):
                        os.mkdir('/seq/vgb/swofford/zoonomia/wholeGenomeAnnotation/orthGeneOut/gtfOut/' + curSpecName)
                    with open('/seq/vgb/swofford/zoonomia/wholeGenomeAnnotation/orthGeneOut/gtfOut/' + curSpecName + '/' + useRefProtId + '.gtf', 'w') as outGTF:
                        transcripts = {}
                        for entry in exonBed:
                            if (int(entry[1]) < int(entry[2])):
                                try:
                                    transcripts[entry[0]]['start'].append(int(entry[1]) + 1)
                                    transcripts[entry[0]]['end'].append(int(entry[2]))
                                    #transcripts[entry[0]]['strand'] = entry[5]
                                except KeyError:
                                    transcripts[entry[0]] = {}
                                    transcripts[entry[0]]['start'] = [int(entry[1]) + 1]
                                    transcripts[entry[0]]['end'] = [int(entry[2])]
                                    transcripts[entry[0]]['strand'] = entry[5]
                                        
                        for contig in transcripts.keys():
                        
                            transcriptMin = min(transcripts[contig]['start'])
                            transcriptMax = max(transcripts[contig]['end'])
                            strand = transcripts[contig]['strand']
                            outGTF.write(contig + '\tzoonomia_pfenning\ttranscript\t'
                                         + str(transcriptMin) + '\t' + str(transcriptMax)
                                         + '\t.\t' + strand + '\t.\t'
                                         + 'gene_id "' + refProtGeneEns + '_' + curSpecName 
                                         + '_zoonomia"; gene_version "1"'
                                         + '; transcript_id "' + useRefProtId + '_' + curSpecName + '_zoonomia"; transcript_version "1"'
                                         + '"; gene_name "' + geneName 
                                         + '"; gene_source "zoonomia"; gene_biotype "protein_coding"; ' 
                                         + 'transcript_source "zoonomia"; ' 
                                         + 'transcript_biotype "protein_coding";\n') 
                        
                        for entry in exonBed:
                            if (int(entry[1]) < int(entry[2])):
                                outGTF.write(entry[0] + '\tzoonomia_pfenning\tCDS\t' 
                                             + str(int(entry[1]) + 1) + '\t' + str(entry[2]) + '\t.\t'
                                             + entry[5] + '\t.\t' + 'gene_id "'
                                             + refProtGeneEns + '_' + curSpecName
                                             + '_zoonomia"; gene_version "1"; transcript_id "'
                                             + useRefProtId + '_' + curSpecName
                                             + '_zoonomia"; transcript_verson "1"; '
                                             + 'exon_number "' + entry[3].split('.e_')[1]
                                             + '"; gene_name "' + geneName + '"; gene_source "zoonomia"; gene_biotype "protein_coding";\n')
